...
export const subtitleMatchWorker = new Worker<SubtitleMatchJobData>(
  'subtitle-match',
  async (job) => {
    Log.info(`subtitle-match job started media_id=${job.data.media_id}`)

    try {
      const matcher = new SubtitleMatcher({
        id: job.data.media_id,
        episode_id: job.data.episode_id,
      })
      ...
      Log.info(
        `subtitle-match job completed media_id=${job.data.media_id} matched=${!!result.matched}`
      )
    } catch (err: any) {
      await DbEvents.create({
        media_id: job.data.media_id,
      ...
      Log.error(
        `subtitle-match job failed media_id=${job.data.media_id} error="${err.message}"`
      )
    }
  }
)
