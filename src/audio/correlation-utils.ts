import type { AudioCorrelator } from '@/audio/audio-correlator'
import { WindowedCorrelator } from '@/core/windowed-correlator'
...
export type Correlate = typeof AudioCorrelator.correlateTimestamps
...
export function scaleTimestamps(timestamps: Float32Array, factor: number) {
...
export function bestCorrelation(
  ...
  const results = speedCandidates(fileSpeed).map((speed) => {
    const scaled = speed === 1 ? target : scaleTimestamps(target, 1 / speed)
...
export function buildRefineRanges(runtimeSeconds: number) {
  ...
  const ranges: { label: string; startSeconds: number; endSeconds: number }[] =
    []
...
export function refineCorrelation(input: {
  reference: Float32Array
  target: Float32Array
  ...
  const windows = WindowedCorrelator.correlate({
    ...
    target: scaleTimestamps(input.target, 1 / input.speed),
    ranges: buildRefineRanges(input.runtimeSeconds),
  ...
  if (denominator <= 0) {
    Log.warn(
      `refineCorrelation skipped: degenerate slope (denominator=${denominator})`
    )
